...
func artifactInputErrors(command string, includeBundle bool) []CommandError {
	errors := []CommandError{
		declaredCommandError(fault.KindInvalidInput, "invalid_arguments", false, "help "+command, "Pass exact catalog and schema-5 specification paths."),
		...
		declaredCommandError(fault.KindNotFound, "specification_file_not_found", false, "help spec validate", "Select an existing schema-5 specification file."),
		...
		declaredCommandError(fault.KindInvalidInput, "invalid_specification_yaml", false, "help spec validate", "Correct the strict schema-5 YAML syntax."),
		declaredCommandError(fault.KindInvalidInput, "legacy_tailoring_schema", false, "help spec init", "Create a schema-5 surface and wrapper specification without automatic conversion."),
...
func bundleFileErrors(command string) []CommandError {
	return []CommandError{
		...
		declaredCommandError(fault.KindInvalidInput, "legacy_tailoring_schema", false, "help bundle build", "Rebuild with a schema-5 specification and bundle schema 4."),
...
func tailoringSpecificationOutputSchema() *OutputSchema {
	...
	fields := []OutputSchemaField{
		...
		array("/commands/*/wrapper/invoke/option_defaults", OutputFieldTypeObject),
		field("/commands/*/wrapper/invoke/option_defaults/*/option", OutputFieldTypeString),
		field("/commands/*/wrapper/invoke/option_defaults/*/value", OutputFieldTypeString),
...
func wrapperPlanOutputSchema() *OutputSchema {
	...
	fields := []OutputSchemaField{
		...
		array("/specification_entry/wrapper/invoke/option_defaults", OutputFieldTypeObject),
		field("/specification_entry/wrapper/invoke/option_defaults/*/option", OutputFieldTypeString),
		field("/specification_entry/wrapper/invoke/option_defaults/*/value", OutputFieldTypeString),
		...
		array("/stages/invoke/applied_option_defaults", OutputFieldTypeObject),
		field("/stages/invoke/applied_option_defaults/*/option", OutputFieldTypeString),
		field("/stages/invoke/applied_option_defaults/*/value", OutputFieldTypeString),
		...
		array("/stages/invoke/option_defaults", OutputFieldTypeObject),
		field("/stages/invoke/option_defaults/*/option", OutputFieldTypeString),
		field("/stages/invoke/option_defaults/*/value", OutputFieldTypeString),
	...
	return CommandSpec{
		...
		Agent: AgentContract{
			...
			Errors: []CommandError{
				...
				declaredCommandError(fault.KindInvalidInput, "legacy_tailoring_schema", false, recovery, "Create or validate a schema-5 tailoring specification; automatic authorization-to-surface conversion is not available."),
...
func DefaultCatalog() Catalog {
	...
	return NewCatalog(
		...
		CommandSpec{
			Path:    "spec init",
			Summary: "Create a schema-5 identity or admitted optimizer baseline",
			Args:    "--catalog <path> [--processor <inspection.json>] -- <command>",
			Effect:  operation.EffectRead,
			Role:    RoleUtility,
			Agent: AgentContract{
				CapabilityID: "tailoring.spec.init",
				Outcome:      "Create an exclude-by-default schema-5 authoring baseline for one exact verified command: identity without processor evidence, or the finite registry-owned optimizer default when one explicit source-compatible processor observation is supplied",
				Inputs: []CommandInput{
				...
				Output: CommandOutput{
					...
					Fields: []OutputField{
						{Name: "specification", Type: OutputFieldTypeObject, Description: "Complete schema-5 YAML tailoring specification authoring baseline; processor evidence may select only a registry-owned finite optimizer default.", Schema: tailoringSpecificationOutputSchema()},
					},
				...
				Errors: append([]CommandError{
					...
					declaredCommandError(fault.KindContract, "invalid_specification_draft", false, "help spec init", "Inspect schema-5 draft construction."),
					declaredCommandError(fault.KindNotFound, "catalog_file_not_found", false, "source inspect", "Generate and select a source inspection JSON file."),
					declaredCommandError(fault.KindPermission, "catalog_file_permission_denied", false, "source inspect", "Correct catalog file permissions."),
					declaredCommandError(fault.KindInvalidInput, "unsafe_catalog_file", false, "source inspect", "Use a stable regular source inspection file."),
					declaredCommandError(fault.KindInvalidInput, "catalog_file_too_large", false, "source inspect", "Regenerate a bounded source inspection file."),
		...
		CommandSpec{
			Path:    "spec validate",
			Summary: "Validate and normalize a catalog-bound schema-5 specification",
			Args:    "--catalog <path> --spec <path>",
			Effect:  operation.EffectRead,
			Role:    RoleUtility,
			Agent: AgentContract{
				CapabilityID: "tailoring.spec.validate",
				Outcome:      "Validate one strict schema-5 YAML tailoring specification against exact source catalog evidence and return its canonical digest and surface-wrapper counts",
				Inputs: []CommandInput{
					{Name: "--catalog", Source: InputSourceFlag, Required: true, ValueKind: InputValueText, Cardinality: InputCardinalitySingle, Description: "Read the exact bounded JSON document emitted by source inspect.", AllowedValues: []string{}},
					{Name: "--spec", Source: InputSourceFlag, Required: true, ValueKind: InputValueText, Cardinality: InputCardinalitySingle, Description: "Read one bounded strict schema-5 tailoring specification.", AllowedValues: []string{}},
				},
				Output: CommandOutput{
					Authority: OutputAuthorityCatalog,
					...
					Fields: []OutputField{
						...
						{Name: "specification", Type: OutputFieldTypeObject, Description: "Normalized vendor-neutral schema-5 tailoring specification with the complete finite authoring inventory.", Schema: tailoringSpecificationOutputSchema()},
					},
					Delivery: OutputDeliveryComplete, CollectionCoverage: CollectionCoverageNotApplicable,
					JSONEnvelope: "validation", JSONSchemaVersion: 2,
				},
				Prerequisites: []string{
					"A reviewed source inspect JSON document and schema-5 YAML specification; validation does not adopt either artifact.",
		...
		CommandSpec{
			...
			Agent: AgentContract{
				...
				Outcome:      "Compile exact source evidence, a valid schema-5 surface-wrapper specification, and exactly required explicit processor evidence into one deterministic identity-bound bundle without adopting or executing it",
				Inputs: []CommandInput{
					{Name: "--catalog", Source: InputSourceFlag, Required: true, ValueKind: InputValueText, Cardinality: InputCardinalitySingle, Description: "Read the exact bounded JSON document emitted by source inspect.", AllowedValues: []string{}},
					{Name: "--spec", Source: InputSourceFlag, Required: true, ValueKind: InputValueText, Cardinality: InputCardinalitySingle, Description: "Read one bounded strict schema-5 tailoring specification.", AllowedValues: []string{}},
				...
				Prerequisites: []string{
					"A source inspect JSON document and schema-5 specification that passes spec validate; build does not create an adoption receipt.",
			...
			Agent: AgentContract{
				...
				Output: CommandOutput{
					...
					Fields: []OutputField{
						...
						{Name: "plan", Type: OutputFieldTypeObject, Description: "Complete schema-6 tailored plan binding source, artifacts, surface, specification entry, argv, stages, processor evidence when present, process framing, and runtime bounds.", Schema: wrapperPlanOutputSchema()},
				...
				Prerequisites: []string{"One current schema-4 bundle whose exact digest is user-adopted; preview revalidates source path, SHA-256, and size and never treats adoption as source authorization."},
			...
			Agent: AgentContract{
				...
				Output: CommandOutput{
					...
					Fields: []OutputField{
						...
						{Name: "plan_digest", Type: OutputFieldTypeString, Description: "SHA-256 identity of the freshly rebuilt schema-6 wrapper plan; it equals preview for identical current inputs."},
				...
				Prerequisites: []string{
					"One current schema-4 bundle whose exact digest is user-adopted; execution rebuilds rather than consumes a preview document.",
		...
		CommandSpec{
			...
			Agent: AgentContract{
				...
				Prerequisites: []string{
					...
					"Contract-3 text output is fixed product source, not specification-authored code; root, included-namespace, and included-command final --help views are compiled from the bundle, while activation and later modification of the bytes remain caller-owned.",
				},
