...
- [x] Add exhaustive preview/execute production-composition recovery and
  packaged-help signature matrices.
...
- [x] Focused tests pass on the final implementation. Evidence: race-enabled
  production CLI, bundle execution, bundle loader, source runner,
  artifact-journey, artifact-evidence, and source-fixture tests pass on Go
  1.26.5.
...
- [x] Every public runtime recovery is phase-accounted. Evidence: 27 preview
  zero-attempt cases, 28 execute pre-start cases, and 15 execute post-start
  cases match exact scoped help; actual source-file drift uses the production
  identity reader, a post-attempt decorator owns defensive execute encoding,
  and infrastructure/application/domain tests own production boundary
  emission.
...
- [x] Generated diff and final repository status are understood. Evidence: the
  recovery implementation and its durable contracts are committed by concern,
  all four profiles pass consecutively, and the tree remains clean.
