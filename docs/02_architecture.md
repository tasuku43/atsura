...
- A wrapper binding contains only the exact adopted purpose bundle, wrapper
  ...
  protocol or configuration state.
...
definition so parsing and later ordinary resolution select the wrapper. It
does not edit any activation file; this in-memory replacement occurs only when
the caller sources the generated bytes.
...
must be one portable POSIX Name outside the maintained reserved/fixed and
implementation-specific function-name set. The runtime derives the same
...
insufficient for an optimizer release claim. Schema 5 retains that identity
case and adds source-catalog schema 2, Go contract 2, processor-observation
schema 1, the exact RTK artifact and contract, distinct bundle and plan
digests, exact caller/source/processor argv, formats, process modes, v2
environment and bounds, optimized and honestly reachable pre-processor
...
installed journey does not claim processor-launch counts without an accepted
