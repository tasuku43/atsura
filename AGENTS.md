...
15. **Wrapper plans describe stages, not permission.** A source wrapper plan binds the adopted bundle and source identity, matched tailored command, original/transformed argv, typed before/output/after stages, mode, specification entry, and reason. It does not require a source read/create/write classification, authorization target/impact, or universal confirmation decision. Host allow/ask/deny values are adapter transport vocabulary only.
