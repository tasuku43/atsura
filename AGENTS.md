...
4. **Every externally visible operation declares an effect.** Use `operation.EffectRead`, `operation.EffectExecute`, `operation.EffectCreate`, or `operation.EffectWrite`; unknown effects fail closed. Execute means starting an external source or processor process behind its declared boundary; it is not a read or a permission decision. Source-owned downstream semantics remain unclassified by Atsura.
...
6. **Each side-effect class crosses one controlled boundary.** Atsura-owned create/write effects cross the central mutation invoker. Source-owned execution crosses a separate identity-bound, no-shell, bounded process port. An admitted external output processor crosses its own identity-bound, no-shell, isolated process port and receives only the bounded stage input Atsura has already validated; it receives no source-selection authority. Do not give a command or use case an unrestricted executor, filesystem, process, or network client.
...
12. **External text remains untrusted data.** Atsura-owned visible projection protects terminal and TSV/JSON structure by distinguishing backslashes, controls/formats, and Unicode line separators; it does not filter printable prompt-like meaning. Opaque references bypass display projection and retain their exact validated value. A plan-declared `source_stream_passthrough` result is a deliberate adopted exception: after a conventionally completed, identity-bound invocation, Atsura may return the bounded source stdout and stderr bytes without projection, framing, UTF-8, terminal-safety, or semantic-safety claims. An `original_preserving_optimizer` is a second finite exception: an ineligible conventional source result may return its exact bytes and status as `preserved_before_processor` without starting the processor, while a successful processor may return only the byte-identical admitted input as `preserved_after_processor` or the independently validated optimized bytes as `optimized`. After processor authority begins, every processor fault suppresses source and processor bytes and never falls back. Neither exception bypasses surface resolution, invocation transformation, source identity, processor identity where applicable, or fresh-plan validation. Atsura does not persist these bytes or embed them in faults. Process uncertainty suppresses captured streams before delivery; final stdout/stderr delivery is non-atomic, may leave partial caller-visible bytes on write failure, and never makes replay safe.
...
15. **Wrapper plans describe stages, not permission.** A source wrapper plan binds the adopted bundle and source identity, any required processor identity, matched tailored command, original/transformed argv, typed before/output/after stages, mode, specification entry, and reason. It does not require a source read/create/write classification, authorization target/impact, or universal confirmation decision. Coding-agent-host permission vocabulary is outside the product model.
...
8. Declare `Effect` and `Intent`. For source or processor process launch, use `EffectExecute`, bind exact executable identity and argv at the owning application/process boundary, use no shell, declare finite attempts/time/bytes, and treat every post-start unknown outcome as non-retryable. A processor additionally receives only the admitted bounded stage input and no source-selection authority. Execute carries no mutation target or impact. For Atsura-owned create/write, additionally declare `TargetRef` and `Impact`, bind create scope through `MutationContract.parent_input`, and bind a write's existing target through `target_id_input` plus any distinct optional parent role. Make missing, unbound, mismatched, or inconsistent values fail before the relevant boundary.
